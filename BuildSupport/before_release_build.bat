...
"C:\Program Files (x86)\Jenkins\fnr.exe" --cl --dir "%WORKSPACE%\GMap.NET.Core\Properties" --fileMask "GlobalAssemblyInfo.cs" --caseSensitive --useRegEx --find "\[assembly: AssemblyInformationalVersion\(\"(\d+(\.\d+)+)(.*?)\"\)\]" --replace "[assembly: AssemblyInformationalVersion(\"$1.%BUILD_NUMBER% Released Version\")]" 